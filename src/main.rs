...
fn main() -> Result<(),ImageDataErrors> {
    ...
    if img_format_2 != img_format_1{
         return Err(ImageDataErrors::DifferentImageFormats);
    }

    let (image_1, image_2) =  standardise_size(img_1, img_2);
    let mut output = FloatingImage::new(image_1.width(), image_1.height(), args.result);

    let combined_image = combine_images(image_1, image_2);
    output.set_data(combined_image)?;  

    image::save_buffer_with_format(output.name, &output.data, output.width, output.height, image::ColorType::Rgba8, img_format_1).unwrap();  
    Ok(())
}
